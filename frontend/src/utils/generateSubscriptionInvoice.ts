...
export const generateSubscriptionInvoice = async (
  ...
  const isPaid = ["completed", "success"].includes(
  ...
  currentY += 5;
  doc.text(
    `Time: ${format(new Date(payment.created_at), "hh:mm aa")}`,
    margins.left,
    currentY,
  );
  currentY += 10;
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    payment.business_name || "Gonzo System User",
    margins.left,
    currentY,
  );
  currentY += 12;
