        ...
        <CardContent className="p-4 pt-0">
          <div className="space-y-3">
            {recentSales.map((sale) => {
              // Use database-provided values directly
              const saleTotal = toSafeNum(sale.total);
              const totalDiscount = toSafeNum(sale.discount);
              const totalCost = toSafeNum(sale.totalCost);
              const profit = toSafeNum(sale.profit);
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {canViewAvgPrice
                          ? `${currency} ${formatNumber(
                              totalQuantity > 0 ? (toSafeNum(sale.subtotal) / totalQuantity) : 0
                            )}`
                          : "NaN"}
                      </p>
                    </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {recentSales.map((sale) => {
                  ...
                  const totalCost = toSafeNum(sale.totalCost);
                  const profit = toSafeNum(sale.profit);
                  const subtotal = toSafeNum(sale.subtotal);
                  const totalQuantity = sale.totalQuantity ?? 0;
                  const itemDescription = sale.itemDescription || "No items";

                  const averagePrice =
                    totalQuantity > 0 ? subtotal / totalQuantity : 0;
