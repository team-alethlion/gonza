...
const BulkTransactionAddTab: React.FC<BulkTransactionAddTabProps> = ({
  ...
  const handleSaveAll = async () => {
    const validRows = rows.filter(
      (r) => r.amount > 0 && r.description.trim() !== "",
    );
    if (validRows.length === 0) {
      alert(
        "Please enter at least one valid transaction with amount and description",
      );
      return;
    }

    // Verify transfers have a destination
    const invalidTransfers = validRows.filter(
      (r) => r.transactionType === "transfer" && !r.toAccountId,
    );
    ...
    try {
      ...
      alert(
        "Failed to save transactions. Please check your network connection.",
      );
    } finally {
      setIsSaving(false);
    }
            ...
            <TableRow
                  ...
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      <span className="truncate">
                        {row.date ? format(row.date, "MMM dd, yyyy") : "Date"}
                      </span>
                    </Button>
                  </PopoverTrigger>
              ...
              <TableCell>
                <Select
                  ...
                  onValueChange={(val: any) =>
                    updateRow(row.id, { transactionType: val })
                  }>
                ...
                {row.transactionType === "transfer" && (
                  <div className="mt-1.5">
                    <Select
                      ...
                      onValueChange={(val) =>
                        updateRow(row.id, { toAccountId: val })
                      }>
              ...
              <TableCell className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateRow(row.id, { personInCharge: e.target.value })
                  }
                  className="text-[10px] h-7 px-1.5"
                />
              </TableCell>
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <DialogDescription className="text-slate-500 dark:text-slate-400 mt-1">
                Add multiple records at once. Each row can have its own date and
                details.
              </DialogDescription>
            </div>
          </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                <Alert className="bg-blue-50/50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900 py-2">
                  ...
                  <AlertDescription className="text-[11px] sm:text-xs">
                    Use the <strong>Default Date</strong> to quickly set dates
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                <div className="flex-1 md:flex-initial">
                  <p className="text-[10px] font-semibold text-muted-foreground uppercase mb-1 ml-1">
                    Default Date
                  </p>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className={cn(
                          "justify-start text-left font-normal w-full md:w-[200px] h-10 shadow-sm",
                          !globalDate && "text-muted-foreground",
                        )}>
                        <CalendarIcon className="mr-2 h-4 w-4 text-blue-500" />
                        {globalDate ? (
                          format(globalDate, "PPP")
                        ) : (
