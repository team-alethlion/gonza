...
import { SoldItem } from "@/hooks/useSoldItemsData";
...
const TopSellingProductsTable: React.FC<TopSellingProductsTableProps> = ({
  ...
  const handleCSVExport = () => {
    ...
    const rows = safeProducts.map((item, index) => [
      (index + 1).toString(),
      item.description,
    ...
    link.download = `top-selling-items-${periodLabel
      .toLowerCase()
  ...
  const handlePDFExport = () => {
    ...
    // Prepare table data
    const tableData = safeProducts.map((item, index) => [
      (index + 1).toString(),
      ...
      item.totalQuantity.toString(),
      canViewSellingPrice
        ? `${settings.currency} ${formatNumber(item.averagePrice)}`
        : "RESTRICTED",
      canViewSellingPrice
        ? `${settings.currency} ${formatNumber(item.totalAmount)}`
        : "RESTRICTED",
      canViewCostPrice
        ? `${settings.currency} ${formatNumber(item.averageCost || 0)}`
        : "RESTRICTED",
      canViewCostPrice
        ? `${settings.currency} ${formatNumber(item.totalCost)}`
        : "RESTRICTED",
      canViewProfit
        ? `${settings.currency} ${formatNumber(item.totalProfit)}`
        : "RESTRICTED",
    ]);
    ...
    // Save the PDF
    doc.save(
      `top-selling-items-${periodLabel.toLowerCase().replace(/\s+/g, "-")}-${
