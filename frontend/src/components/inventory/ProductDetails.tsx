...
const ProductDetails: React.FC<ProductDetailsProps> = ({
  ...
  const { createStockHistoryEntry, stockHistory: allStockHistory } =
    useStockHistory(user?.id, product.id);
  ...
  const getInitialStockDate = (): Date | null => {
    const historyToCheck =
      stockHistory.length > 0 ? stockHistory : allStockHistory;
  ...
  const validateAdjustmentDateTime = (
    ...
    if (proposedDateTime < initialStockDate) {
      return `Date and time cannot be before the initial stock date and time (${formatDate(
        initialStockDate,
        "PPP p",
      )})`;
    }
    ...
    try {
      ...
      if (adjustmentType === "Stock In") {
        ...
        if (newQuantity === 0 && previousQuantity < adjustmentQuantity) {
          toast.warning(
            `Attempted to remove ${adjustmentQuantity} items but only ${previousQuantity} were available. Stock is now 0.`,
          );
        }
      }
        ...
        <div className="flex flex-col space-y-3 lg:flex-row lg:justify-between lg:items-center lg:space-y-0 lg:gap-4">
          ...
          <div className="flex flex-col space-y-2 lg:flex-row lg:items-center lg:space-y-0 lg:gap-2">
            ...
            {hasPermission("inventory", "edit") && (
              <Button
                onClick={() =>
                  router.push(`/agency/inventory/edit/${product.id}`)
                }
                ...
                <div className="flex flex-col items-center justify-center py-6 bg-gray-50 rounded-lg border border-dashed">
                  {product.barcode ? (
                    <div className="bg-white p-6 shadow-sm rounded-md flex flex-col items-center">
                      <span className="text-lg font-bold mb-2 truncate max-w-[280px]">
                        {product.name}
                      </span>
                      <Barcode
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Include price on the printed label
                      </p>
                    </div>
                    <Switch
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        fetch("http://localhost:5000/print/label", {
                          ...
                          body: JSON.stringify({
                            PrinterName:
                              settings.defaultPrinterName || "Label Printer",
                          ...
                          .catch((err) => {
                            ...
                            toast.error(
                              "Printing failed. Make sure Printer Bridge is running.",
                            );
                          });
                      }}>
                      <Printer className="h-4 w-4" />
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className={product.imageUrl ? "md:w-2/3" : "w-full"}>
                ...
                <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
                  ...
                  <dd>
                    {canViewCostPrice
                      ? `${settings.currency} ${formatFinancial(
                          product.costPrice,
                          "cost",
                        )}`
                      : "•••"}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-2">
                      ...
                      <Button
                        type="button"
                        variant={
                          adjustmentType === "Stock Out" ? "default" : "outline"
                        }
