...
const StockInputHistory = () => {
    ...
    try {
      ...
      const groupedBulkHistory = bulkStockAdditions.reduce((groups, entry) => {
        try {
          ...
          if (!groups[sessionId]) {
            const supplierMatch =
              entry.changeReason?.match(/Purchase: ([^|]+)/);
      ...
      const filteredBulkGroups = sortedBulkGroups.filter((group) => {
        ...
        try {
          ...
          const matchesSupplier =
            group.supplier?.toLowerCase()?.includes(searchLower) || false;
        ...
        try {
          ...
          return group.entries.some((entry) => {
            try {
              if (!entry?.productId) {
                return (
                  entry.changeReason?.toLowerCase()?.includes(searchLower) ||
                  false
                );
              }
              const productDetails = getProductDetails(entry);
              const matchesProduct = productDetails.name
                .toLowerCase()
  ...
  const getProductDetails = useCallback(
    (entry: StockHistoryEntry) => {
      try {
        ...
        // 1. Prioritize Snapshotted Price (Highest Accuracy)
        if (costPrice !== undefined && costPrice > 0) {
          return {
            name:
              backendProduct?.name ||
              (changeReason?.match(/^\[(.*?)\]\s*\|/)?.[1]) ||
              "Deleted Product",
            costPrice: costPrice,
        ...
        // 2. Use Backend-provided fallback (Highly Reliable, no race condition)
        if (backendProduct && backendProduct.costPrice > 0) {
          return {
        ...
        // 3. Fallback to current product price from client cache (if loaded)
        const cachedProduct = products.find(
          (p) => p.id === productId || p.itemNumber === productId,
        );
        if (cachedProduct) {
          return {
            name: cachedProduct.name,
            costPrice: cachedProduct.costPrice || 0,
            sellingPrice: cachedProduct.sellingPrice || 0,
          };
        }

        // 4. Last resort: Scavenge from reason or return placeholder
        if (changeReason) {
        ...
        return {
          name: `Deleted Product (${
            idStr.length > 8 ? idStr.slice(-8) : idStr
          })`,
          costPrice: 0,
          sellingPrice: 0,
        };
  ...
  const calculateGroupTotals = (entries: StockHistoryEntry[]) => {
    return entries.reduce(
      (totals, entry) => {
        const productDetails = getProductDetails(entry);
        const qtyAdded = (entry.newQuantity || 0) - (entry.oldQuantity || 0);

        // Use snapshotted cost price OR current cost price from lookup
        const costPrice = entry.costPrice || productDetails.costPrice || 0;
        const amount = qtyAdded * costPrice;
  ...
  const handleEditStockHistory = async (
    ...
    const success = await updateStockHistoryEntry(
      entryId,
      newQuantity,
      newReason,
      newDate,
  ...
  const handleEditGroupDateConfirm = async (newDate: Date) => {
    ...
    const productIds = Array.from(
      new Set(editGroupDateDialog.group.entries.map((e) => e.productId)),
    );

    try {
      const result = await updateStockHistoryDatesAction(
        entryIds,
        newDate.toISOString(),
      );
      ...
      clearAllLocationCaches();
      await Promise.all([loadStockHistory(), loadProducts()]);
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>
              {searchTerm.trim()
                ...
                : "No stock input history found for the selected date range."}
            </p>
          </div>
        ) : (
          <TooltipProvider>
                            ...
                            <Table>
                              ...
                              <TableBody>
                                {group.entries.map((entry) => {
                                  ...
                                  const productExistsInCache = products.some(
                                    (p) =>
                                      p.id === entry.productId ||
                                      p.itemNumber === entry.productId,
                                  );
                                  ...
                                  return (
                                    <TableRow key={entry.id}>
                                      ...
                                      <TableCell>
                                        {canViewCostPrice
                                          ? `${currencySymbol}${formatFinancial(
                                              price,
                                              "cost",
                                            )}`
                                          : "Null"}
                ...
                <div>
                  <div className="mb-4">
                    <h3 className="text-lg font-semibold text-muted-foreground">
                      Regular Stock Additions
                    </h3>
                    <p className="text-sm text-muted-foreground">
                      Individual stock additions and manual adjustments
                    </p>
                  </div>
