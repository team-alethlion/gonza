      ...
      <div
        ...
        <div className="flex flex-col items-center mb-4 text-center">
          {settings.businessLogo && (
            <div className="mb-2 w-full flex justify-center">
              <Image
                ...
                alt="Logo"
                // Assuming a 1:1 or 4:1 ratio; these props help Next.js
                // calculate the space, but the CSS below will govern the size.
                width={150}
                height={48}
                className="h-12 w-auto max-w-[40mm] object-contain"
                // Since it's a business logo in settings/header, priority is best
                priority
              />
            </div>
          )}
          <div className="text-[14px] font-black uppercase tracking-tight mb-0.5">
          ...
          {settings.businessEmail && (
            <div className="text-[9px] font-bold opacity-80 lowercase">
              {settings.businessEmail}
            </div>
          )}
        </div>
        ...
        <div className="text-center mb-3">
          <div className="text-[12px] font-black underline mb-1">
            {getDocumentTitle()}
          </div>
          ...
          {sale.items.map((item: any, index: any) => {
            ...
            return (
              <div key={index} className="mb-2 last:mb-0">
                <div className="font-bold text-[10px] break-words uppercase">
                  {item.description}
                </div>
                <div className="flex justify-between text-[9px] items-center">
                  ...
                  <span className="font-bold">
                    {formatNumber(item.quantity).padStart(2, "0")}x{" "}
        ...
        <div className="mb-4 space-y-0.5">
          <div className="flex justify-between font-bold">
            <span>SUBTOTAL:</span>
            <span>
              {displayCurrency} {formatNumber(subtotalBeforeDiscount)}
            </span>
          </div>
        ...
        <div className="mb-4 text-[9px] italic opacity-80 border-t border-b border-gray-100 py-1">
          <span className="font-black not-italic uppercase">Words:</span>{" "}
          {displayCurrency} {numberToWords(totalAmount)} only.
        </div>

        {/* Payment Info */}
        {hasPaymentInfo && (
          <div className="mb-4 text-[9px]">
            <div className="font-black underline mb-1 uppercase">
              Payment Channels:
            </div>
            {paymentMethods
              .filter(
              ...
              .map((p, i) => (
                <div key={i} className="mb-1 last:mb-0 font-bold">
                  ...
                  <div className="text-[8px] opacity-70 italic text-right">
                    {p.accountName}
                  </div>
                </div>
              ))}
        ...
        {sale.notes && (
          <div className="mb-4 text-[9px] border border-gray-100 p-1 bg-gray-50">
            ...
            <div className="font-bold opacity-70 whitespace-pre-wrap italic">
              {sale.notes}
            </div>
          </div>
        )}
          ...
          {settings.signature ? (
            <div className="flex flex-col items-center">
              <Image
                ...
                alt="Signature"
                // Signatures are usually wider than they are tall (e.g., 4:1 ratio)
                width={160}
                height={40}
                // h-10 is 40px in Tailwind
                className="h-10 w-auto object-contain mb-1 mix-blend-multiply opacity-90"
                // Signatures on documents are critical for "above the fold" rendering
                priority
              />
