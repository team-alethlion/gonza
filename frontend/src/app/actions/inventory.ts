...
export async function deleteStockHistoryEntriesByReferenceAction(referenceId: string, locationId: string) {
    try {
        await verifyBranchAccess(locationId);
        await djangoFetch(`inventory/producthistory/delete_by_reference/?referenceId=${referenceId}&locationId=${locationId}`, { method: 'DELETE' });
...
export async function getStockSummaryReportAction(locationId: string, startDate: string, endDate: string) {
    try {
        await verifyBranchAccess(locationId);
        const data = await djangoFetch(`inventory/products/summary_report/?locationId=${locationId}&startDate=${startDate}&endDate=${endDate}`);
...
export async function getSoldItemsReportAction(branchId: string, startDate: string, endDate: string) {
    try {
        await verifyBranchAccess(branchId);
        const data = await djangoFetch(`inventory/products/sold_items/?branchId=${branchId}&startDate=${startDate}&endDate=${endDate}`);
        return { success: true, data };
    } catch (error: any) {
        return { success: false, error: error.message };
    }
...
export async function getStockTransfersAction(branchId: string) {
    try {
        await verifyBranchAccess(branchId);
        const result = await djangoFetch(`inventory/stock-transfers/?from_branch=${branchId}`);
        return { success: true, data: result };
    } catch (error: any) {
        return { success: false, error: error.message };
    }
