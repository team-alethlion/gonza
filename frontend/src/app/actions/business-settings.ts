...
export async function getBusinessSettingsAction(
  ...
  try {
    ...
    if (error.message?.includes("Session stale")) {
      console.warn(
        `[SettingsAction] Request blocked: Session is orphaned (Redirection expected).`,
      );
    } else {
      console.error(
...
export async function upsertBusinessSettingsAction(
  ...
  try {
    ...
    const existingData = Array.isArray(existing)
      ? existing[0]
      : existing.results?.[0];
    ...
    if (error.message?.includes("Session stale")) {
      console.warn(`[SettingsAction] Upsert blocked: Session is orphaned.`);
    } else {
      console.error(
        "Error upserting business settings:",
        error.message || error,
      );
    }
    return {
...
export async function completeInitialOnboardingAction(data: any) {
  try {
    ...
    if (error.message?.includes("Session stale")) {
      console.warn(`[OnboardingAction] Blocked: Session is orphaned.`);
    } else {
      console.error(
        "Error completing initial onboarding:",
        error.message || error,
      );
    }
    return {
...
export async function getAccountStatusAction(userId: string, session?: any) {
  try {
    ...
    if (error.message?.includes("Session stale")) {
      console.warn(
        `[AccountStatus] Request blocked: Session is orphaned (Redirection expected).`,
      );
    } else {
...
export async function getOnboardingStatusAction(locationId: string) {
  try {
    ...
    const settings = Array.isArray(existing)
      ? existing[0]
      : existing.results?.[0];
    ...
    if (error.message?.includes("Session stale")) {
      console.warn(`[OnboardingStatus] Request blocked: Session is orphaned.`);
    } else {
      console.error(
