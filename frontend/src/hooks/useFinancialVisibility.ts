...
export const useFinancialVisibility = () => {
    ...
    const canViewCostPrice = isPowerUser || hasPermission('inventory', 'view_cost_price');
    const canViewProfit = isPowerUser || hasPermission('inventory', 'view_profit');
    const canViewSellingPrice = isPowerUser || hasPermission('inventory', 'view_selling_price');

    // Dashboard-specific permissions
    const canViewTotalSales = isPowerUser || hasPermission('dashboard', 'view_total_sales');
    const canViewTotalGrossProfit = isPowerUser || hasPermission('dashboard', 'view_gross_profit');
    const canViewTotalExpenses = isPowerUser || hasPermission('dashboard', 'view_total_expenses');
    const canViewInventoryValue = isPowerUser || hasPermission('dashboard', 'view_inventory_value');
    const canViewSalesTypes = isPowerUser || hasPermission('dashboard', 'view_sales_types');
    const canViewAvgPrice = isPowerUser || hasPermission('dashboard', 'view_avg_price');
    const canViewTotalAmount = isPowerUser || hasPermission('dashboard', 'view_total_amount');

    // Finance and Expenses permissions
    const canManageFinanceAccounts = isPowerUser || hasPermission('finance', 'manage_accounts');
