...
### 📡 Payload Bloat: Redundant Sales Fetching
In `frontend/src/hooks/useDashboardData.ts`:
*   **The Code**: `const { sales } = useSalesData(user?.id, 'desc', 50, true, initialSales);`
*   **The Issue**: This hook fetches 50 full `Sale` objects from the backend (or SSR). Each `Sale` object includes a nested `items` array with full product details (description, quantity, price, cost).
*   **Redundancy**: The dashboard only uses this list for:
    1.  Calculating `nonQuoteSalesCount` (total count of paid/pending sales).
    2.  Passing to the `RecentSalesTable`.
*   **The Better Way**: 
    1.  The backend `analytics/summary/` endpoint **already provides** `paidSalesCount` and `pendingSalesCount`.
    2.  The backend `analytics/summary/` endpoint **already provides** a list of the 20 most recent sales (`recentSales`).
*   **Impact**: We are downloading ~200-500KB of redundant data (50 full sales) on every dashboard mount when the summary object (~5KB) already contains everything needed for the overview.
